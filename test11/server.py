...
def search_keyword():
    ...
    try:
        ...
        print(f'X-API-KEY (길이: {len(headers["X-API-KEY"])}): ...{headers["X-API-KEY"][-5:]}')
        print(f'X-Customer: {headers["X-Customer"]}')
...
if __name__ == '__main__':
    ...
    print(f'Customer ID: {CUSTOMER_ID if CUSTOMER_ID else "미설정"}')
    print(f'API Key 길이: {len(NAVER_ACCESS_LICENSE)} 문자')
    print(f'API Key 끝 5자: ...{NAVER_ACCESS_LICENSE[-5:] if NAVER_ACCESS_LICENSE else "없음"}')
    print('=' * 50)
