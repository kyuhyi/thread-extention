...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  chrome.storage.onChanged.addListener((changes, area) => {
    if (area === 'local' && changes.contentGenerationState?.newValue) {
      applyGenerationState(changes.contentGenerationState.newValue).catch(error => showStatus(error.message, 'error'));
    }
  });
  const { state } = await chrome.runtime.sendMessage({ action: 'getContentGenerationState' });
  if (state) await applyGenerationState(state);
  else {
    const saved = (await chrome.storage.local.get('lastGeneratedContent')).lastGeneratedContent;
    if (saved) {
      try { displayResults(ContentContract.validate(saved)); }
      catch (_) { showStatus('이전 글 응답이 완성되지 않았습니다. 다시 생성해주세요.', 'warning'); }
    }
  }
});

let appliedGenerationId = null;
async function applyGenerationState(state) {
  ...
  if (state.status === 'error') {
    BlogImages.suspend();
    const message = state.error || '글 생성을 완료하지 못했습니다. 다시 시도해주세요.';
    [elements.shortsText, elements.blogText, elements.threadText].forEach(el => { el.textContent = message; el.dataset.plainText = ''; });
    elements.resultSection.classList.remove('hidden');
    showStatus(message, 'error');
    return;
  }
  if (state.status !== 'completed' || appliedGenerationId === state.id) return;
  const data = ContentContract.validate(state.data);
  appliedGenerationId = state.id;
  displayResults(data);
  if (BlogImages.getArticleText() !== data.blog_post) {
    await BlogImages.setArticle(data.blog_post, { auto: true });
  }
}
...
async function generateContent() {
  try {
    ...
    if (response.success) {
      await applyGenerationState(response.state);
...
function displayResults(data) {
  data = ContentContract.validate(data);
  // 각 콘텐츠를 HTML로 포맷팅하여 표시
  const shortsText = ShortsPost.format(data.shorts_script);
  elements.shortsText.innerHTML = formatTextToHTML(shortsText);
  elements.shortsText.dataset.plainText = shortsText;
  if (BlogImages.getArticleText() !== data.blog_post) elements.blogText.innerHTML = formatTextToHTML(data.blog_post);
  const threadText = ThreadPost.format(data.thread_post);
  elements.threadText.innerHTML = threadText.split('\n\n').map(p => `<p>${BlogDocument.escape(p).replace(/\n/g, '<br>')}</p>`).join('');
  elements.threadText.dataset.plainText = threadText;
  document.querySelectorAll('[data-target="shorts"], [data-target="thread"]').forEach(button => { button.disabled = false; });
...
function formatTextToHTML(text) {
  ...
  const formatted = paragraphs
    ...
    .map(paragraph => {
      // 단일 줄바꿈은 <br>로 변환
      const lines = BlogDocument.escape(paragraph.trim()).replace(/\n/g, '<br>');

      // 해시태그 스타일링
      const styledLines = lines.replace(/(^|\s)(#[\p{L}\p{N}_]+)/gu, '$1<span class="hashtag">$2</span>');
...
async function copyContent(target) {
  if (target === 'blog') return BlogImages.copyAll();
  let element;
  ...
  const text = element.dataset.plainText ?? (element.innerText || element.textContent);
...
function showLoading() {
  BlogImages.suspend();
  elements.shortsText.textContent = '숏츠 글 생성 중…';
  elements.shortsText.dataset.plainText = '';
  elements.blogText.textContent = '블로그 글 생성 중…';
  elements.threadText.textContent = '스레드 글 생성 중…';
  elements.threadText.dataset.plainText = '';
  elements.resultSection.classList.remove('hidden');
  document.querySelectorAll('[data-target="shorts"], [data-target="thread"]').forEach(button => { button.disabled = true; });
